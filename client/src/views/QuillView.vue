<template>
  ...
  <div v-else>Click "Start Meeting" to get started!</div>
</template>
...
const colors = ["#09f", "#0f9", "#90f", "#9f0", "#f09", "#f90"];
...
let SocketProvider: WebrtcProvider | null = null;

function initials(str) {
  return str
    .split(" ")
    .slice(0, 2)
    .map((el) => el.substr(0, 1).toUpperCase())
    .join("");
}
...
onMounted(async () => {
  if (route.params.meetingId) {
    meetingId.value = <string>route.params.meetingId;

    user.name = Math.floor(Math.random() * 100000).toString();
    ...
    let res: GetUserContextResponse | null = null;
    let meetingUUIDRes: GetMeetingUUIDResponse | null = null;
    ...
    meetingUUIDRes = await zoomSdk.getMeetingUUID();

    console.log({ res, user });

    meetingId.value = (meetingUUIDRes.meetingUUID || "waffles").replace(
      /[^\d\w]/gi,
      ""
    );
  }
